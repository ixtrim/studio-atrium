					...
					{if $_item.attachments.CarouselBg[0]}
						<div data-b="0" data-p="170.00" data-po="80% 55%" style="display: none;" {if $idx != 0} class="sl" {/if}>
							{if $_item.link}<a href="{$_item.link}">{/if}
								<img data-u="image"
									data-src2="{$stockPath}/{$_item.attachments.CarouselBg[0].path}/{$_item.attachments.CarouselBg[0].filename}"
									alt="{$_item.name}" />
								{if $_item.text1 || $_item.text2 || $_item.text3}
									<div class="box">
										{if $_item.text1}<div data-u="caption" data-t="0" class="subbox1">
												<strong{if $_item.extra_data.text1_bg || $_item.extra_data.text1_opacity}
													class="slideText1_{$_item.id}" {/if}>{$_item.text1}</strong>
										</div>{/if}
										{if $_item.text2}<div data-u="caption" data-t="1" class="subbox2">
												<span{if $_item.extra_data.text2_bg || $_item.extra_data.text2_opacity}
													class="slideText2_{$_item.id}" {/if}>{$_item.text2}</span>
										</div>{/if}
										{if $_item.text3}<div data-u="caption" data-t="2" class="subbox3">
												<span{if $_item.extra_data.text3_bg || $_item.extra_data.text3_opacity}
													class="slideText3_{$_item.id}" {/if}>{$_item.text3}</span>
										</div>{/if}
									</div>
								{/if}
								{if $_item.attachments.CarouselImage[0]}
									<div class="img" data-u="caption" data-t="3">
										<div
											style="width: {$_item.attachments.CarouselImage[0].props.image_size.width}px; height: {$_item.attachments.CarouselImage[0].props.image_size.height}px;">
											<img src="{$stockPath}/{$_item.attachments.CarouselImage[0].path}/{$_item.attachments.CarouselImage[0].filename}"
												alt="{$_item.title}"
												width="{$_item.attachments.CarouselImage[0].props.image_size.width}"
												height="{$_item.attachments.CarouselImage[0].props.image_size.height}" />
										</div>
									</div>
								{/if}
								{if $_item.link}
	...
	<section class="sneakPeak">
		...
		<p>Jesteśmy autorską pracownią projektową z <strong>30 letnim</strong> stażem. To tylko dowodzi, że projekty
			...
			pracująca w branży projektowo-budowlanej ekipa doradców z dużym doświadczeniem, chętnie Ci w tym pomoże! <a
				href="{url module=varia action=project_helper}">Znajdziemy dla Ciebie projekt &raquo;</a></p>
	</section>
</div>
<div class="container">
	<section>
				...
				{if $smarty.foreach.bbb.first}
					<figure class="effect-sadie">
						<img class="lazy-image"
							data-uri="{$stockPath}/{$_item.attachments.BoxBg[0].path}/{$_item.attachments.BoxBg[0].filename}"
							src="img/xc.png" width="480" height="480" alt="{$_item.name}">
					...
					{if !$_item.bg_color}
						<figure class="effect-sadie">
							<img class="lazy-image"
								data-uri="{$stockPath}/{$_item.attachments.BoxBg[0].path}/{$_item.attachments.BoxBg[0].filename}"
								src="img/xc.png" width="480" height="480" alt="{$_item.name}">
							{if $_item.name}<div class="mobile-sadie" 
								{if $_item.id==37 || $_item.id==21}
									style="background-color: rgba(255,0,0,0.75);" {/if}><span>{$_item.name}</span></div>
							{/if}
							<span class="close-sadie"></span>
							...
							<figcaption>
								...
								<a class="framed" href="{$_item.link}">
									<span>Zobacz
										wszystkie{if $_item.project_category_id && $siteMenuStats[$_item.project_category_id]}
										{$siteMenuStats[$_item.project_category_id]}{/if}</span>
								</a>
							</figcaption>
						</figure>
					{else}
						<figure class="{'off mainPageBox_'|cat:$_key}">
							...
							<div>
								<img class="lazy-image"
									data-uri="{$stockPath}/{$_item.attachments.BoxImage[0].path}/{$_item.attachments.BoxImage[0].filename}"
									src="img/xc.png" alt="{$_item.name}" width="1" height="1">
											...
											{foreach $_item.content as $comment}
												...
												<li>
													<p class="comment-title"><a
															href="{url module=discuss action=thread id=$threadId}">{$topic|strip_tags|truncate:24}
															| {$comment.nick}</a></p>
													<p><a
															href="{url module=discuss action=thread id=$threadId}">{$comment.content|strip_tags|truncate:100}</a>
													</p>
												</li>
											{/foreach}
										</ul>
									...
									<div class="flexslider">
										<ul class="slides">
											{foreach $_item.content as $article}
												<li>
													<p class="title"><a
															href="{url module=article action=item docId=$article.id link_title=$article.title}">{$article.title}</a>
													</p>
													<p><a
															href="{url module=article action=item docId=$article.id link_title=$article.title}">{$article.teaser|strip_tags|truncate:100}
		...
		<section class="featured">
			<div class="list-grid fav-wrapper" id="overlay-group">
				...
				{foreach $featured as $_project}
					<div>
						<figure>
							<img class="lazy-image" data-uri="{image type=render project=$_project size=box}" src="img/xc.png"
								alt="Projekt domu {$_project.name}" width="640" height="427">

							<figcaption>
								<a
									href="{url module=project action=item id=$_project.id link_title=$_project.name catalog='projekty-domow'}">
									<span>projekt domu
										{if $_project.params_general|hasFloor:true}piętrowego
										{elseif $_project.params_general|hasLoft:true}z
											poddaszem{elseif $_project.params_general|isGroundFloor:true}parterowego{/if}{if $isLocal}
										{$_project.symbol_alpha} {$_project.symbol_num}{/if}</span>
									<strong>{$_project.name} <span>{$_project.params_general|usableArea}
											m<sup>2</sup></span></strong>
								</a>
							</figcaption>
						</figure>

						<span class="overview" data-id="{$_project.id}" data-idx="{$_project@index}"
							data-img="{image type=render project=$_project size=presentation}"
							data-ground="{image type=sketch project=$_project}" {if $_project.params_general|hasFloor:true}
								data-floor="{image type=sketch project=$_project storey=1st_floor}"
								{/if}{if $_project.params_general|hasLoft:true}
							data-loft="{image type=sketch project=$_project storey=loft}" {/if}
							data-link="{url module=project action=item id=$_project.id link_title=$_project.name catalog='projekty-domow'}"
							data-price="{if $_project.price}{if $_project.discount}<strike>{$_project.price}</strike> {$_project.price-$_project.discount}{else}{$_project.price}{/if}{else}-{/if}"
							data-name="{$_project.name}" data-area="{$_project.params_general|usableArea}"
							data-parcel="{$_project.params_general|parcelWidth} x {$_project.params_general|parcelHeight}"
							data-height="{$_project.params_general|houseHeight}"
							data-angle="{$_project.params_general|roofAngle}"
							data-version="{if $_project.type == 'skeleton'}wersja szkieletowa{else}wersja murowana{/if}"
							data-rooms="{$_project.params_general|roomCount}" data-txt="{$_project.short_description}"></span>
						<span id="compare-{$_project.id}" class="compare{if in_array($_project.id, $compareIds)} on{/if}"
							data-id="{$_project.id}"></span>
						<span id="fav-{$_project.id}" class="fav{if in_array($_project.id, $favouriteIds)} on{/if}"
							data-id="{$_project.id}"></span>

						{if $_project|isNew || $_project.discount}
							<div>
								{if $_project.discount}<span class="discount">rabat
										{$_project.discount}</span>{/if}{if $_project|isNew}<span class="new">nowość</span>{/if}
								</div>
							{/if}
						</div>
					{/foreach}
...
<section class="real">
	<div class="real-grid" id="real-grid">
		{foreach $realisations as $_item}
			...
			<figure class="real">
				<img class="lazy-image"
					data-uri="{$projectPath}/{$_item.path}/{$_item.filename|replace:"realizacja-":"realizacja-317-"}"
					src="img/xc.png" width="475" height="317" alt="Projekt {$_item.object.name} - realizacja">

				<figcaption>
					<a
						href="{url module=project action=item id=$_item.object.id link_title=$_item.object.name catalog='projekty-domow'}#realizacje">
						<span>projekt domu
							{if $_item.object.params_general|hasFloor:true}piętrowego
							{elseif $_item.object.params_general|hasLoft:true}z
							poddaszem{elseif $_item.object.params_general|isGroundFloor:true}parterowego
							{/if}</span>
						<strong>{$_item.object.name} {$_item.object.params_general|usableArea}
		...
		{foreach $interiors as $_item}
			...
			<figure class="real">
				<img class="lazy-image"
					data-uri="{$projectPath}/{$_item.path}/{$_item.filename|replace:"budowa-":"budowa-317-"}"
					src="img/xc.png" width="475" height="317" alt="Projekt {$_item.object.name} - realizacja wnętrza">
		...
		{foreach $unfinished as $_item}
			...
			<figure class="real">
				<img class="lazy-image"
					data-uri="{$projectPath}/{$_item.path}/{$_item.filename|replace:"budowa-":"budowa-317-"}"
					src="img/xc.png" width="475" height="317" alt="Projekt {$_item.object.name} w budowie">
	...
	<div>
		{foreach $partners as $_partner}
			{if $_partner.title == 'Hörmann' || $_partner.title == 'Fakro' || $_partner.title == 'Termo Organika' || $_partner.title == 'Aluprof'}
				<a href="/dokumenty/Wspolpraca.html#par{$_partner.id}"><img class="lazy-image"
						data-uri="{articleImage document=$_partner}" src="img/xc.png" alt="{$_partner.title}" width="1"
						height="1"></a>
			{/if}
		{/foreach}
