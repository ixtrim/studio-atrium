		...
		<ul>
			...
			<li>
				<ul class="tooltip-box" id="tooltip-box" style="display: none;">
					<li>
						<a class="tooltip{if !$user} consultant{/if}"
							href="{if $user}{if $project}{url module=panel action=message project_id=$project.id}{else}{url module=panel action=message}{/if}{else}javascript:{/if}">
							...
							<ul class="menu-base">
								{foreach $siteMenu.house as $_item}
									{if $_item.menu_position == 1 && $_item.is_highlight}
										<li>
											<a
												href="/{$_item.link}{if strpos($_item.link, '.html') === false}/{/if}">{$_item.name}</a>
										</li>
									{/if}
								{/foreach}
								<li class="catalog-box"><a href="{url module=catalog action=form}"><img
											src="/img/catalogue.webp" alt="Katalog projektów domów"></a></li>
								<li><a href="{url module=catalog action=form}" class="framed">Zamów bezpłatny
										katalog</a></li>
								<li class="play">
									<a
								...
								<ul class="menu">
									{foreach $siteMenu.house as $_item}
										{if $_item.menu_position == $smarty.section.menu.iteration && $_item.children}
											<li{if $_item.children} class="header" 
											{elseif $_item.is_highlight} class="highlight"
												{/if}>
												{if $_item.children}
													...
													<ul class="submenu">
														{foreach $_item.children as $_subitem}
															<li><a
																	href="/{$_subitem.link}{if strpos($_subitem.link, '.html') === false}/{/if}">{$_subitem.name}</a>
															</li>
														{/foreach}
													</ul>
												{else if $_item.link}
			...
			<ul class="menu-box" id="forums-menu" style="display: none;">
				<li class="full">
					<ul class="forum-menu-box">
						...
						<li>
							<a href="{url module=discuss action=category id=100}" class="fcat-ask">Pytania do
								projektu</a>
						</li>
						<li>
							<a href="{url module=discuss action=category id=1}" class="fcat-sa">Budowa wg projektu
								Studia Atrium</a>
						</li>
						<li>
						...
						<li>
							<a href="{url module=discuss action=category id=3}" class="fcat-misc">Tematy
								ogólnobudowlane</a>
						</li>
						<li>
							<a href="{url module=discuss action=category id=4}" class="fcat-interior">Urządzanie wnętrz
								i&nbsp;użytkowanie</a>
						</li>
					</ul>
				</li>
				...
				<div class="max-w-[1480px] mx-auto px-8 py-8 max-h-[min(72vh,680px)] overflow-y-auto">
					...
					<div class="site-mega-content hidden" data-mega-panel="projekty">
						<div class="grid grid-cols-1 md:grid-cols-[220px_repeat(3,minmax(0,1fr))] gap-10 lg:gap-14">
							...
							{section name=col loop=3 start=1}
								<div class="space-y-1">
									{foreach $siteMenu.house as $_item}
					...
					<div class="site-mega-content hidden" data-mega-panel="garaze">
						<div class="grid grid-cols-1 sm:grid-cols-3 gap-10 lg:gap-16 py-2">
							{section name=garcol loop=3 start=1}
								<div>
									{foreach $siteMenu.other as $_item}
										{if ($smarty.section.garcol.iteration == 1 && $_item.menu_position == 1)
											|| ($smarty.section.garcol.iteration == 2 && $_item.menu_position == 2)
											|| ($smarty.section.garcol.iteration == 3 && $_item.menu_position >= 3)}
											{if $_item.children}
