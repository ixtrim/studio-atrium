                        ...
                        <div class="swiper-slide !h-auto">
                            <a href="{$item.url|escape}"
                                ...
                                <div class="relative overflow-hidden">
                                    ...
                                    {if $item.tag}
                                        <span
                                            class="absolute top-3 left-3 text-[11px] font-bold tracking-wider bg-white/90 text-[var(--brand-red)] px-2.5 py-1">{$item.tag|escape}</span>
                                    {/if}
                                </div>
                                <div class="px-5 pt-4 pb-5 flex flex-col gap-3 flex-1">
                                    ...
                                    <div class="text-[13px] font-bold tracking-wider text-[var(--brand-red)]">
                                        {$item.type_label|escape}</div>
                                    <div class="flex items-center gap-4 py-2 text-[13px] text-[#222]">
                                        {if $item.area}
                                            <div class="flex items-center gap-2"><span
                                                    class="w-[32px] h-[32px] border border-black/20 inline-flex items-center justify-center shrink-0 text-[#555]"><svg
                                                        xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                                        stroke-linecap="round" stroke-linejoin="round"
                                                        class="lucide lucide-vector-square" aria-hidden="true">
                                                    ...
                                                    </svg></span><span>{$item.area|escape}</span></div>
                                        {/if}
                                        {if $item.rooms != ''}
                                            <div class="flex items-center gap-1.5"><span
                                                    class="w-[32px] h-[32px] border border-black/20 inline-flex items-center justify-center shrink-0 text-[#555]"><svg
                                                        ...
                                                        <path d="M6 8v9"></path>
                                                    </svg></span><span>{$item.rooms|escape}</span></div>
                                        {/if}
                                        {if $item.baths > 0}
                                            <div class="flex items-center gap-1.5"><span
                                                    class="w-[32px] h-[32px] border border-black/20 inline-flex items-center justify-center shrink-0 text-[#555]"><svg
                                                    ...
                                                    </svg></span><span>{$item.baths|escape}</span></div>
                                        {/if}
                                        {if $item.garage > 0}
                                            <div class="flex items-center gap-1.5"><span
                                                    class="w-[32px] h-[32px] border border-black/20 inline-flex items-center justify-center shrink-0 text-[#555]"><svg
                                                    ...
                                                    </svg></span><span>{$item.garage|escape}</span></div>
                                        {/if}
                                    </div>
                                    <div class="pt-1 mt-auto">
                                        {if $item.price_old}
                                            <div class="text-[16px] text-[var(--brand-red)] line-through">
                                                {$item.price_old|escape} PLN</div>
                                        {/if}
                                        <div class="flex items-baseline gap-2"><span
                                                class="text-[34px] font-['Montserrat',sans-serif] font-semibold text-[var(--brand-blue-strong)] leading-none">{$item.price|escape}</span><span
                                                class="text-[16px] text-[var(--brand-blue-strong)] font-semibold">PLN</span>
                                        </div>
                                    </div>
                                </div>
                            </a>
